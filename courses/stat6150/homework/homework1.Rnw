...
\begin{document}
\lhead{Homework 1}
\chead{STAT 6150 - Advanced Bayesian Methods}
...
\item Consider the model $y_{i,g} \stackrel{ind}{\sim} N(\theta_g, \sigma^2)$.
  \begin{enumerate}
  \item Let $\theta_g\stackrel{ind}{\sim} Ho(0,\tau^2)$ be the horseshoe prior described in Carvalho et al. (2009). Describe an MCMC algorithm to sample from this model including the steps for the algorithm as well as the full conditionals (when available) or the method you are using to sample from each step.
  \item Construct a simulated data set (you get to decide the number of groups, at least 5, and number of observations in each group).
  \item Compare the results of the Horseshoe prior to results from using a point-mass prior.
  \end{enumerate}
