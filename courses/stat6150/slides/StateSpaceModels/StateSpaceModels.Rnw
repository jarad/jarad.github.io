...
\multicolumn{2}{l}{\mbox{Observation equation:}} \\
 \uncover<2->{Y_t &= f_t(\theta_t,v_t)} & \uncover<4->{Y_t|\theta_t &\ind p_t(y_t|\theta_t,\dots)} \\ 
 \\
\multicolumn{2}{l}{\mbox{State transition (evolution) equation:}} \\
 \uncover<3->{\theta_t &= g_t(\theta_{t-1},w_t)} & \uncover<5->{\theta_t|\theta_{t-1} &\ind p_t(\theta_t| \theta_{t-1},\ldots)}
 \end{array} \]
...
\subsection{Examples}
\frame{\frametitle{Stochastic volatility model}
...
\pause
But, in most models the form stays the same and only the state changes with
time.
...
p(y_{t+k}|y_{1:t}) \pause &=& \int p(y_{t+k}|\theta_{t+k})p(\theta_{t+k}|y_{1:t}) p\theta_{t+k} \\
 \end{eqnarray*}
}
}
...
\begin{itemize}
\item What is forecast distribution at time $t$ given only $\pi_0^0 = \pi_0$,
\pause
i.e. $\pi_t^0$?
\pause
Recursively, we have 
...
i.e. what are the $\pi_{t,s}^{t}$ for all states $s$?
\pause
Find this recursively via
\[
\pi_{t,s}^{t} \propto p(y_t|\theta_{t} = s) \pi_{t-1}^{t-1} \cdot P_{\cdot,s}
\]
% \pause
...
$P(\theta_T = s|y_{1:T}) = \pi_{T,s}^T$. 
\item For $t=T,\ldots,1$, sample $\theta_{t-1}$ from a discrete distribution 
with 
\[
P(\theta_{t-1} = s|\theta_t,y_{1:t-1}) 
\propto P_{s,\theta_t} \pi_{T-1,s}^{T-1}
= \frac{P_{s,\theta_t} \pi_{T-1,s}^{T-1}}{\sum_{s'=1}^S P_{s',\theta_t} \pi_{T-1,s'}^{T-1}}.
\]
\end{enumerate}
\end{frame}
...
we have 
\[ 
\sum_{s'=1}^S \Psi_{ss'} = 1 \quad \forall\, s.
\]
...
\pause
i.e. let $\Psi_{s\cdot}$ be the $i$th row of $\Psi$, then
\[ 
\Psi_{s\cdot} \sim Dir(A_s)
\]
where $A_s$ is a vector of length $S$ and $A$ is the matrix with rows $A_s$.
...
The Dirichlet distribution (named after Peter Gustav Lejeune Dirichlet), i.e. $P\sim Dir(a)$, is a probability distribution for a probability vector of length $S$. \pause The probability density function for the Dirichlet distribution is 
\[
p(P;a) = \frac{\mG(a_1+\cdots+a_S)}{\mG(a_1)\cdots \mG(a_S)} \prod_{s=1}^S p_h^{a_s-1}
\]
where $p_s\ge 0$, $\sum_{s=1}^S p_s=1$, and $a_s>0$. 

\vspace{0.2in} \pause

Letting $a_0 = \sum_{s=1}^S a_s$, then some moments are
\begin{itemize}[<+->]
\item $E[p_s] = \frac{a_s}{a_0}$,
\item $V[p_s] = \frac{a_s(a_0-a_s)}{a_0^2(a_0+1)}$,
\item $Cov(p_s,p_{s'}) = -\frac{a_sa_{s'}}{a_0^2(a_0+1)}$, and 
\item $\mbox{mode}(p_s) = \frac{a_s-1}{a_0-S}$ for $a_s>1$.
\end{itemize}

A special case is $S=2$ which is the beta distribution.
...
Let $A$ be the hyperparameter with rows $A_s$ such that 
\[ 
\Psi_i \stackrel{ind}{\sim} Dir(A_s)
\]
\pause
and
$C$ be the count matrix of observed transitions, 
\pause
i.e. $C_s$ is the count vector of transitions from $s$ to all states and 
$C_{ss'}$ is the count of transitions from $s$ to $s'$. 
...
that 
\[ 
\Psi_s|y \stackrel{ind}{\sim} Dir(A_s') \stackrel{d}{=} Dir(A_s+C_s)
\]
\pause
...
If we assume $\Psi_s \ind Dir(A)$, 
\pause
then a Gibbs sampling approach is
\begin{enumerate}
\item Sample $\theta_{1:t}|\Psi,y \sim \prod_{t=1}^T p(\theta_{t-1}|\theta_t,y_{1:t},\Psi)$.
\item For $s=1,\ldots,S$, sample $\Psi_s|\theta,y \ind Dir(A_s+C_s)$ where 
$C_s$ is the count vector of transitions from $s$ to all states.
\end{enumerate}
