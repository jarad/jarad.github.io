...
\frametitle{Inference}
	...
	\pause but the likelihood is the solution to the \alert{chemical master equation}:
	\[
...
For constitutive production, $h(X_t)=1$ and $a(X_t)=\theta$, \pause
	we still have 
...
\frametitle{Gibbs sampling}
	Our objective is samples from the posterior 
\[ 
p(\theta,X|y) \propto p(y|X)p(X|\theta)p(\theta) \pause = 
\left[ \prod_{t=1}^n \mathrm{I}(y_t=X_t) \right] p(X|\theta)p(\theta) 
\]
where $p(\theta|y)$ is the marginal of $p(\theta,X|y)$.
