...
\begin{frame}[fragile]
    ...
    \[ P(Z>4.5) \approx \frac{1}{J} \sum_{m=1}^M \mathrm{I}(Z_m>4.5)\]
    is a standard MC approach. \pause
<<mc-tail-sampling>>=
...
\frame{\frametitle{}
    ...
    p(\theta_t|y_{1:t}) &\approx&  \sum_{i=1}^J w_t^{(m)} \delta_{\theta_t^{(m)}} \pause
    \end{eqnarray*}
...
for (t in 2:T) {
  particles[[t]] <- data.frame(
    ...
    # New step
    theta = sample(particles[[t-1]]$theta, 
...
for (t in 2:T) {
  # Resample
  indices <- sample(1:M, M, replace = TRUE,
                    prob = dnorm(d$y[t], 
                                 particles[[t-1]]$theta, 
                                 sd = sqrt(W + V)))
  ...
  particles[[t]] <- data.frame(
    ...
    # Move
    theta = rnorm(M, particles[[t-1]]$theta[indices])) 
}
