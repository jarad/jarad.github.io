...
\begin{definition}
A process $Y(s)$ is \alert{isotropic} if the semivariogram function depends only on $||h||$, the length of the separation vector. Otherwise the process is \alert{anisotropic} if it depends on the direction (rather than just the length/distance.
\end{definition}
...
\[
p(\mu,\nu,\phi,\tau^2,\sigma^2|y) \propto N_n(y;\mu,\mySigma)p(\mu,\nu, \phi, \tau^2, \sigma^2)
\]
\pause
...
Consider point-referenced data at spatial locations $s_1,\ldots,s_n$, model these data as
...
\begin{frame}
\frametitle{Bayesian estimation for spatial random effects}

Let $\theta = (\beta, \sigma^2, \tau^2, \phi)$, 

then parameter estimates may be obtained from the posterior distribution:

\[ p(\theta|y) \propto p(y|\theta) p(\theta) \]

\pause where

\[ Y|\theta \sim N_n(X\beta, \sigma^2 H(\phi)+\tau^2\mathrm{I}).\]
...
g = ggplot(d, aes(East_m, North_m, size=DBH_cm)) 
...
\begin{frame}[fragile]
\frametitle{Monte Carlo Standard Error}
<<theta_mcmcse, dependson='spLM_run', echo=TRUE>>=
r$p.theta.samples |>
  as.data.frame() |>
  summarize(across(everything(), mcmcse::mcse))
@
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Monte Carlo Standard Error}
<<beta_mcmcse, dependson='spLM_run', echo=TRUE>>=
r$p.beta.samples |>
  as.data.frame() |>
  summarize(across(everything(), mcmcse::mcse))
@
...
\begin{enumerate}
\item Run the MCMC sampler to obtain draws $(\sigma^2,\phi)^{(m)} \sim p(\sigma^2,\phi|y)$
\item After burn-in and for $m=1,\ldots,M$, sample $w^{(m)}\sim p(w|(\sigma^2,\phi)^{(m)},y)$.
\end{enumerate}
\end{frame}
...
For prediction at points $s_{01},\ldots,s_{0L}$ and denoting $Y_0=(Y(s_{01}),\ldots,Y(s_{0L}))^\top$ and design matrix $X_0$ having rows $x(s_{0\ell})^\top$, \pause we have the following relationship
\[ 
p(y_0|y,X,X_0) = 
\int p(y_0|y,\theta,X_0) p(\theta|y,X) d\theta \pause 
\approx \frac{1}{M} \sum_{m=1}^M p(y_0|y,\theta^{(m)},X_0). 
\]
\pause It is more common to take draws $y_0^{(m)} \sim p(y_0|y,\theta^{(m)},X_0)$ and estimate the predictive distribution using
\[ p(y_0|y,X,X_0)  \approx \frac{1}{M}  \sum_{m=1}^M \delta_{y_0^{(m)}} \]
...
For GLMs (other than linear models), $w(s)$ cannot be integrated out 
\pause 
and therefore a common Gibbs sampling strategy is
\begin{enumerate}
...
\[
Y \sim N_n(0, \tau^2[\rho(D_w-W)+(1-\rho)\I]^{-1})
\]
\pause
