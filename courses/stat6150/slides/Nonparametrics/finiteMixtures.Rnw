...
If we count the number of times consumers choose each category, i.e. 
\[ 
...
<<rnaseq_plot, dependson='rnaseq', message=FALSE, fig.keep='last', fig.height=10, fig.width=12,warning=FALSE>>=
parms = names(hat)[-1]
plots = list()
...
A finite mixture model for the marginal distribution for $Y_i=\hat\phi_i$ is
...
\begin{frame}[fragile]
\frametitle{Convergence diagnostics - Potential Scale Reduction Factor}
...
\begin{frame}[fragile]
\frametitle{Convergence diagnostics - Traceplots}
...
model {
  ...
  mu[1:H] <- sort(mu0)         # sort the means
  pi ~ ddirich(a)
}"
@
...
<<posterior_draws, dependson='jags2_run'>>=
extract_draws <- function(samples) {
  # samples is an mcmc.list object
  draws_list <- lapply(seq_along(samples), function(chain_idx) {
    chain_matrix <- as.matrix(samples[[chain_idx]])
    n_iter <- nrow(chain_matrix)
    
    # Create a data.frame for this chain
    df_chain <- data.frame(
      chain       = chain_idx,
...
draws <- extract_draws(r) 
saveRDS(draws, file='jags2_finite_mixture_draws.rds')
@

<<posterior_density, dependson='posterior-draws'>>=
d <- draws |>
  select(starts_with('mu['), starts_with('sigma['), starts_with('pi[')) |>
  mutate(row_id = row_number()) |>        # optional: keep original row id
  crossing(xx = seq(-3, 10, by = 0.1)) |>
  mutate(
    density = `pi[1]`*dnorm(xx, `mu[1]`, `sigma[1]`) + 
              `pi[2]`*dnorm(xx, `mu[2]`, `sigma[2]`) + 
              `pi[3]`*dnorm(xx, `mu[3]`, `sigma[3]`)
  )
...
<<clustering_results, dependson='jags2_run'>>=
zeta <- draws |>
...
draws <- zeta |>
  pivot_longer(cols = everything(), 
               names_to = 'parameter', 
               values_to = 'value') |>
  mutate(parameter = gsub('zeta\\[|\\]', '', parameter),
...
<<clustering_final, dependson=c("clustering_results","jags_data"),fig.height=3>>=
cluster <- sm |>
...
Clustering is extremely sensitive to the parametric model chosen, e.g. normal in this example, and the cluster could change dramatically with a different choice, e.g. $t$. 
\end{frame}
...
A model for the joint distribution for $Y_i=(\phi_i,\psi_i)^\top$ is
...
\[ \begin{array}{rl}
\pi &\sim Dir(a) \\
\mu_h|\Sigma_h & \stackrel{ind}{\sim} N_p(\mu_h, v_h^2\mySigma_h) \\
\mySigma_h &\stackrel{ind}{\sim} IW(D_h,c_h)
\end{array} \]
where $c_h>p-1$ is the degrees of freedom and $D$ is the scale matrix. 
\pause 
The mean of this distribution is $D_h/(c_h-p-1)$ for $c_h>p+1$. 
...
jm = jags.model(textConnection(joint_mixture_model), 
                data = dat, 
                n.chains = 3)
r = coda.samples(jm, variable.names = c('pi','mu','Sigma'), n.iter = 1e4)
@
\end{frame}
