...
\section{Bayesian density estimation}

\begin{frame}
\frametitle{Bayesian density estimation}

There are two main approaches to Bayesian density estimation
\begin{itemize}[<+->]
...
\begin{frame}
\frametitle{Bayesian density estimation through Dirichlet Process mixtures}

Key phrases:
\begin{itemize}
\item Dirichlet process (DP)
\item Dirichlet process mixture (DPm)
...
<<rnaseq-plot, dependson='rnaseq',echo=FALSE>>=
ggplot(hat, aes(x = phi)) + 
  geom_histogram(aes(y=after_stat(density)), binwidth=0.2) +
  labs(x = expression(hat(phi)), 
       y = "Density") 
@
\end{frame}
...
\pause 
That is, we want to make prior statements like
\[ 
P(Y_i \in A) = \mathcal{G}(A)
\]
\pause
and posterior statements like 
\[ 
P\left(\tilde{Y} \in A|y\right) = \mathcal{G}_y(A)
\]
for any set $A$. 
\end{frame}
...
One approach is to use a Dirichlet process (Ferguson 1973). 
\pause 
We write 
\[ 
...
\item $G_0$ is the base (probability) measure. 
\end{itemize}

\pause

For any finite partition $A_1,\ldots,A_K$ of the sample space $S$, 
\pause 
the probability vector $[\mathcal{G}(A_1),\,\ldots,\,\mathcal{G}(A_K)]$ 
\pause 
follows this Dirichlet distribution 
\[ 
[\mathcal{G}(A_1),\,\ldots,\,\mathcal{G}(A_K)] \sim Dir(\,[aG_0(A_1),\,\ldots,\,aG_0(A_K)]\,).
\]
Thus for any set $A$ in the partition:
\begin{itemize}[<+->]
\item $E[\mathcal{G}(A)] = G_0(A)$ and 
\item $Var[\mathcal{G}(A)] = \frac{G_0(A)[1-G_0(A)]}{1+a}$.
\end{itemize}
\end{frame}
...
If $\mathcal{G}\sim DP(aG_0)$, 
then the stick-breaking representation of $\mathcal{G}$ is
\[ 
\mathcal{G}(A) 
= \sum_{h=1}^\infty \pi_h \delta_{\theta_h}(A) \pause
= \sum_{h=1}^\infty \pi_h \mathrm{I}(\theta_h \in A) 
\]
\pause
...
text(mp, -0.1, expression(pi[1], pi[2], pi[3], pi[4], pi[5], ...))
text(mp,  0.1, expression(theta[1], theta[2], theta[3], theta[4], theta[5], ...))
@
...
<<stick_breaking_realizations, dependson='stick_breaking', warning=FALSE>>=
...
ggplot(d, aes(x=theta, weight=pi)) +
  ...
  facet_grid(alpha ~ .n, scales='free_y', labeller = label_both) + 
  stat_function(fun=dnorm, color='red') +
  labs(title = "Approximation to stick-representation with 10,000 draws",
       subtitle = "Base measure is a standard normal (density in red)",
       x = expression(theta),
       y = "Density")
@
\end{frame}
...
The stick-breaking representation shows that a DP is almost surely discrete.
\pause
Any data we model where we use a DP prior on the data distribution, 
will result in a discrete distribution for the data. 
\pause 
To model continuous data, we may want to use a \alert{DP mixture}, i.e. 
\[ 
...
Using the stick-breaking construction, we have 
\[ 
Y_i \ind p(\cdot|\theta_i), 
\qquad 
...
For some $\epsilon>0$, 
there exists an $H$ such that $P\left(\sum_{h=H}^\infty \pi_h < \epsilon\right) \approx 1$ and 
components beyond $H$ can reasonably be ignored with the remaining probability
given to component $H$. 

\vspace{0.2in} \pause 

Thus, a finite approximation to the stick-breaking representation is
\[ 
...
\begin{frame}
\frametitle{Recall the RNA-seq example}
<<echo=FALSE>>=
<<rnaseq-plot>>
@
\end{frame}
...
A DP mixture model for the marginal distribution for $Y_i=\hat\phi_i$ is

\[ 
Y_i \ind N(\mu_i, \sigma_i^2) \quad 
\left(\begin{array}{cc} \mu_i \\ \sigma_i^2 \end{array} \right) \ind \sum_{h=1}^H \pi_h \delta_{(\mu_h^*,\sigma_h^{2*})}
\]
...
r = jags.samples(jm, 
                 variable.names = c('mu','sigma','pi','zeta'), 
                 n.iter = 1e3) # up to 1e4
@
\end{frame}
...
As previously discussed, the model as constructed has identifiability problems 
...
\]
where $H^{(-i)}$ is the number of components without the $i$th observation and $n_h^{(-i)}$ is the number of observations in each component without the $i$th observation. 
\end{frame}
...
\begin{itemize}
\item bayesm
\item DPpackage (has been removed)
\item growcurves
\item PReMiuM
\end{itemize}
