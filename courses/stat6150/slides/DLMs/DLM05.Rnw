...
Suppose at each time point, you have observations from multiple units, 
e.g. people, countries, stocks, etc. 
...
Y_{i,t} &= F\theta_t^{(i)}+v_{i,t}, &v_{i,t}\ind N(0,V_i) \\
\theta_t^{(i)} &= G\theta_{t-1}^{(i)}+w_t^{(i)}, & w_t^{(i)}\ind N_p(0,W_i)
\end{array} \]
...
Seemingly unrelated time series (SUTSE)
}
...
Suppose you have a linear trend model for each unit $i$:
 \[ \begin{array}{rl@{\qquad}l}
 Y_{i,t} &= F_{i,t}\theta_t^{(i)} + v_{i,t} &v_{i,t}\ind N_m(0,V_{i,t}) \\
 \theta_{i,t} &= G_{i,t}\theta_{t-1}^{(i)}+w_{t}^{(i)} & w_t^{(i)}\ind N_p(0,W_{i,t})
 \end{array} \]
...
Combine these into a DLM with $m=2$:
 \[ \begin{array}{rl@{\qquad}l}
 Y_{t} &= F_{t}\theta_{t} + v_{t} &v_{t}\ind N_m(0,V_{t}) \\
 \theta_{t} &= G_{t}\theta_{t-1}+w_{t} & w_{t}\ind N_p(0,W_{t})
 \end{array} \]
...
\[ \begin{array}{rl@{\qquad}l}
 Y_{t} &= (F\otimes I_m) \theta_{t} + v_{t} &v_{t}\ind N_m(0,V) \\
 \theta_{t} &= (G\otimes I_m)\theta_{t-1}+w_{t} & w_{t}\ind N_p(0,W)
 \end{array} \]
...
\[ \begin{array}{rl@{\qquad}l}
Y_{i,t} &= F_t^{(i)}\theta_t^{(i)}+v_{i,t}, &v_{i,t}\ind N(0,V_i) \\
\theta_t^{(i)} &= G_{i,t}\theta_{t-1}^{(i)}+w_t^{(i)}, & w_{t}^{(i)}\ind N_p(0,W_i)
\end{array} \]
...
\[ Y_{i,t} = F_{i,t}\theta_{i,t}+v_{i,t}, \quad v_{i,t} \ind N(0,\sigma_{i,t}^2) \]
...
Instead:
\[ \begin{array}{rl@{\qquad}l}
\theta_{i,t} &= \lambda_t +\epsilon_{i,t}, & \epsilon_{i,t}\ind N_k(0,\Sigma_t) \\
\uncover<4->{\lambda_t &= G\lambda_{t-1} + w_t, & w_t \ind N_k(0,W_t)}
\end{array} \]
...
Hierarchical DLM:
\[ \begin{array}{rl@{\qquad}l}
Y_{t} &= F_{y,t}\theta_{t}+v_{t}, & v_{t} \ind N_m(0,V_{y,t}) \\
\theta_{t} &= F_{\theta,t}\lambda_t +\epsilon_{t}, & \epsilon_{t}\ind N_P(0,V_{\theta,t}) \\
\lambda_t &= G_t\lambda_{t-1} + w_t, & w_t \ind N_k(0,W_t)
\end{array} \]
}
}
}
...
\[ \begin{array}{rl@{\qquad}l}
Y_{t} &= F_{y,t}F_{\theta,t}\lambda_{t}+v_{t}^*, & v_{t}^* \ind N_m(0,F_{y,t}V_{\theta,t}F_{y,t}^\top +V_{y,t}) \\
\lambda_t &= G_t\lambda_{t-1} + w_t, & w_t \ind N_k(0,W_t)
\end{array} \]
}
...
\end{array}\] \pause
Then
\[ Y_{i,t} = \sum_{j=1}^k \beta_{j,t} h_j(x_i) +\epsilon_{i,t}, \qquad \epsilon_{i,t}\ind N(0,\sigma^2). \] \pause
In matrix notation,
\[ Y_t = F\beta_t+\epsilon_t, \qquad \epsilon_t\ind N_m(0,V). \] \pause
What are $F$ and $V$?
}
}
...
\[ \begin{array}{rl@{\qquad}l}
Y_t &= F\theta_t+\epsilon_t, & \epsilon_t\ind N_m(0,V) \\
\theta_t &= G\theta_{t-1}+w_t, & w_t \ind N_p(0,W_t)
\end{array} \]
...
Y_t &= A\mu_t+v_t, & v_t&\ind N_m(0,V) \\
\mu_t &= \mu_{t-1}+w_t, & w_t \ind N_p(0,W)
\end{array} \]
...
\[ \begin{array}{rl@{\qquad}ll}
Y_t &= \tilde{A}\tilde{\mu}_t+v_t, & v_t&\ind N_m(0,V) \\
\tilde{\mu}_t &= \tilde{\mu}_{t-1}+\tilde{w}_t, & \tilde{w}_t \ind N_p(0,HWH^\top )
\end{array} \]\pause
