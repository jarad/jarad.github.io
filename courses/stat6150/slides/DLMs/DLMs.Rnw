...
\setkeys{Gin}{width=\textwidth}
  \[ \begin{array}{rl@{\qquad}rl}
 Y_t &= \theta_t + v_t &v_t &\ind N_1(0,V) \\
 \theta_t &= \theta_{t-1}+w_t & w_t &\ind N_1(0,W) \\
...
\frame{\frametitle{Latent state prior}
 ...
 Evolution equation: $\theta_t = G_t\theta_{t-1}+w_t$ where $w_t\ind N_p(0,W_t)$
 (independent of $p(\theta_{t-1}|y_{1:t-1})$. \pause
...
\frame{\frametitle{One-step ahead prediction}
 ...
 Observation equation: $y_t = F_t\theta_t+v_t$ where $v_t\ind N_m(0,V_t)$ (independent of $p(\theta_t|y_{1:t-1})$. \pause
...
\frame{\frametitle{Latent state posterior - linear regression approach}
 ...
 Observation equation: $y_t = F_t\theta_t+v_t$ where $v_t\ind N_m(0,V_t)$
 (independent of $p(\theta_t|y_{1:t-1})$. \pause
...
\frame{\frametitle{Kalman filter}
 ...
 (high values have more weight on the current observation while low values have more weight on the prior information).
}
...
% \frame{\frametitle{Kalman filter (with missing observations)}
%  \begin{itemize}
%  \item Assume $p(\theta_{t-1}|y_{1:t-1})=N(m_{t-1},C_{t-1})$.
%  \item Obtain prior $p(\theta_t|y_{1:t-1}) = N(a_t,R_t)$ where
%  \[ a_t = G_tm_{t-1} \quad \mbox{and} \quad R_t =G_tC_{t-1}G_t^\top +W_t.\]
...
\frame{\frametitle{Kalman filter \alert{with missing data}}
 ...
 e_t &=& y_t-f_t  & K_t &=& R_tF_t^\top Q_t^{-1}
 \end{array} \]
  \item<2-> \alert{If $y_t$ is not observed, $m_t=a_t$ and $C_t=R_t$.}
  \end{itemize}
 \end{itemize}
}
...
p(\theta_0) &\pause=& N_p(m_0,C_0) \\ \pause
\\
...
\[ \begin{array}{rl@{\qquad}ll}
Y_t &= F_t\theta_t + v_t &v_t&\ind N_m(0,\phi^{-1}\tilde{V}_t) \\
\theta_t &= G_t\theta_{t-1}+w_t & w_t&\ind N_p(0,\phi^{-1}\tilde{W}_t) \\
&&\theta_0 &\sim N_p(m_0,\phi^{-1}\tilde{C}_0) \\
...
Let $\psi$ represent any unknown, non-dynamic model parameters such that the data follows a DLM conditional on $\psi$.\pause
 \[ \begin{array}{rl@{\qquad}ll}
 Y_t &= F_t(\psi)\theta_t + v_t &v_t&\ind N_m(0,V_t(\psi)) \\
 \theta_t &= G_t(\psi)\theta_{t-1}+w_t & w_t&\ind N_p(0,W_t(\psi)) \\
 p(\theta_0) &= N_p(m_0(\psi),C_0(\psi))
 \end{array} \]\pause
...
\frame{\frametitle{Local level model - unknown variances}
 \[ \begin{array}{rl@{\qquad}ll}
 Y_t &= \theta_t + v_t &v_t&\ind N_m(0,V) \\
 \theta_t &= \theta_{t-1}+w_t & w_t&\ind N_p(0,W) \\
 p(\theta_0) &= N(m_0,C_0)
 \end{array} \]
...
Let's look at an example: local level model.
 ...
 \[ \begin{array}{rl@{\qquad}ll}
 Y_t &= \theta_t + v_t &v_t&\ind N(0,2) \\
 \theta_t &= \theta_{t-1}+w_t & w_t&\ind N(0,0.5) \\
 p(\theta_0) &= N(0,1)
 \end{array} \]
