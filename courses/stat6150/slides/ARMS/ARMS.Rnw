...
A function $p(\theta)$ is \alert{concave} if 
...
If $p(x)$ is twice differentiable, 
then $p(x)$ is \alert{concave} if and only if $p''(x) \le 0 \, \forall \, x$.

A function $p(x)$ is \alert{log-concave} if $\log p(x)$ is concave.
\end{frame}
...
\item Log-concave densities
  ...
  \end{itemize}
\item Non-log-concave densities
  \begin{itemize}
  \item Log-normal
  \item Student $t$
  \item $F$-distribution
