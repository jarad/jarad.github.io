...
For simplicity, let $V = C = 1$ and $m = 0$, 
then $\theta|y \sim N(y/2, 1/2)$. 
\pause
Suppose \alert{$y = 1$}, then we have
...
\begin{frame}
\frametitle{Normal model with normal prior (cont.)}
Now suppose \alert{$y = 10$}, then we have
...
\[ 
p(\theta|y) = 
e^{-(y-\theta)^2/2V} \left( 1+\frac{1}{v}\frac{(\theta-m)^2}{C}\right)^{-(v+1)/2}
\left/ \int e^{-(y-\theta)^2/2V} \left( 1+\frac{1}{v}\frac{(\theta-m)^2}{C}\right)^{-(v+1)/2} d\theta
\right.
\]
}
\end{frame}
...
\item $t_v(\theta;m,C)$ is the location-scale $t$ density with degrees of freedom $v$, location $m$, and scale $\sqrt{C}$ evaluated at $\theta$
\end{itemize}
\pause
...
# A location-scale t distribution
...
\begin{frame}
\frametitle{Normal model with $t$ prior (cont.)}
Let $v = 1$, $m = 0$, $V = C = 1$ and \alert{$y = 1$}. 
\pause Then
...
\begin{frame}
\frametitle{Normal model with $t$ prior (cont.)}
Let $v=1$, $m=0$, $V=C=1$, and \alert{$y = 10$}. 
\pause 
Then

<<normal_model_t_prior3>>=
y  = 10
py = integrate(function(theta) f(theta, y), -Inf, Inf)
...
<<normal_model_map>>=
range_y <- 5
map_t <- Vectorize(function(y) {
  optimize(function(theta) f(theta, y), 
               c(y - range_y, y + range_y), 
...
ggplot(map, aes(x        = y, 
                ...
                linetype = model)) +
  geom_line() +
  labs(x = expression(y), y = expression(hat(theta))) +
  scale_color_manual(values = colors, limits = force) +
  scale_linetype_manual(values = linetypes, limits = force)
@
...
For simplicity, let $b=V=1$, $m=0$ and \alert{$y = 1$}.
\pause
Then
...
For simplicity, let $b = V = 1$, $m = 0$ and \alert{$y = 10$}.
\pause
Then
...
d = adply(unique(map$y), 1, function(d){
  o = optimize(function(x) f(x, d, V, m, b, log = TRUE), 
               c(-mx, mx), maximum = TRUE)
...
\alert{Can we construct a prior such that the posterior has non-zero density at zero?}
...
For simplicity, let $V=C=1$, $p=0.5$, $m=0$ and \alert{$y = 1$}. 
\pause 
Then
...
For simplicity, let $V=C=1$, $p=0.5$, and $m=0$
and \alert{$y = 10$}.
\pause
Then
...
Suppose $v=V=C=1$, $p=0.5$, $m=0$, and \alert{$y = 1$}.
...
Suppose $v = V = C = 1$, $p = 0.5$, and $m = 0$ and \alert{$y = 10$}.
\pause
Then
