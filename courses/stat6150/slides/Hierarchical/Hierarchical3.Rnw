...
\item $X$ is a known $n\times p$ model matrix 
\item $\sigma^2$ is an unknown scalar
\end{itemize}

\vspace{0.1in} \pause

For a given model matrix $X$, we are interested in the posterior 
\[ p(\beta,\sigma^2|y), \]
\pause
...
ggplot(d, aes(x = temp, y = chirps)) + geom_point()
@
\end{frame}
...
<<lm, echo=TRUE>>=
summary(m <- lm(chirps ~ temp, data = d))
...
tmp = lm(chirps ~ temp, data = d) # default analysis
tmp$coefficients
confint(tmp)
...
\begin{frame}[fragile]
\frametitle{Shrinkage (as $Var[\beta_1]$ gets smaller)}
<<shrinkage, fig.width=10>>=
dV = plyr::ddply(data.frame(Variance = 10^seq(-2, 2, by = 0.2)), 
                 .(Variance), 
                 function(x) {
  m = bayesglm(chirps ~ temp, 
               data        = d, 
               prior.mean  = 0, 
               prior.scale = x$Variance, 
               prior.df    = Inf)
  data.frame(beta0 = m$coefficients[1], beta1 = m$coefficients[2])
})
tmp = dV |> pivot_longer(-Variance, names_to = "variable", values_to = "estimate")

ggplot(tmp, aes(Variance, estimate)) +
  geom_line() +
  scale_x_log10() + 
...
ggplot(d, aes(x = temp, y = chirps)) + 
  geom_point() + 
  geom_abline(data = dV, aes(intercept = beta0, slope = beta1, color = Variance)) + 
...
If we choose the conjugate prior $\beta \sim N(b_0, \sigma^2 B_0)$ 
(independent of $\epsilon$), 
we still need to choose $b_0$ and $B_0$. 
\pause 
It seems natural to set $b_0=0$ which will shrink the estimates 
for $\beta$ toward zero, i.e. toward no effect. 
\pause 
But how should we choose $B_0$?

\vspace{0.1in} \pause 

One option is \alert{Zellner's $g$-prior} where $B_0 = g[X^\top X]^{-1}$ 
\pause 
where $g$ is either set or learned.
...
\[ 
\beta|\sigma^2,y \sim 
N\left(\frac{1}{1+g}b_0 + \frac{g}{1+g}\hat{\beta}_{MLE}, \frac{\sigma^2 g}{g+1}[X^\top X]^{-1} \right) 
 \]
...
\[ \beta \sim N(b_0, g\sigma^2 [X^\top X]^{-1}), p(\sigma^2)\propto 1/\sigma^2 \]
we need to determine how to set or learn $g$.
...
We use the marginal likelihood as evidence in favor of the model, 
i.e. when comparing models those with higher marginal likelihoods should be 
...
By Bayes' rule, the \alert{posterior model probability} is 
\[ 
p(M_k|y) 
\pause 
= \frac{p(y|M_k)p(M_k)}{p(y)} 
\pause
= \frac{p(y|M_k)p(M_k)}{\sum_{j=1}^J p(y|M_j)p(M_j)} 
\pause 
= \left[1 + \sum_{j\ne k} \frac{p(y|M_j)p(M_j)}{p(y|M_k)p(M_k)}\right]^{-1}
\]
\pause
...
The \alert{marginal likelihood} is 
\[
p(y|M_k) = \int p(y|\theta, M_k) p(\theta|M_k) p\theta
\]
for the observed value of $y$ 
\pause
and the \alert{Bayes factor for model $M_k$ over $M_{k'}$} is 
\[
BF(M_k:M_1) = \frac{p(y|M_k)}{p(y|M_{k'})}.
\]
\pause
For equal prior probability, 
higher marginal likelihood, $p(y|M)$,
results in higher posterior model probability, $p(M|y)$. 

\end{frame}


\begin{frame}
\frametitle{Compared to other model selection criterion}
...
multiply by -2 and plugging in $\theta = \hat\theta_{MLE}$:
\[
...
% 
% \begin{tabular}{cc}
% Criteria & $2\times$ Penalty \\
% \hline
% AIC & k \\
% BIC & \log(n) k / 2 \\
% % DIC* & \\
% ML  & \log\left(\frac{p(\hat\theta_{MLE}|y,M) }{p(\hat\theta_{MLE}|M) }\right) \\
% WAIC & 
% \end{tabular}
% \end{frame}
...
If $\beta$ is a vector of length $p$, 
let $\gamma$ be a vector with binary elements that indicate whether that 
component of $\beta$ is non-zero, 
i.e. that explanatory variable is included. 
\pause 
For example, 
...
Then we have $X_\gamma$ which indicates the model matrix that only has columns corresponding to those columns in $\gamma$ that are non-zero \pause and $\beta_\gamma$ is the subset of $\beta$ including elements of $\beta$ where $\gamma$ is 1. \pause
...
If we want to compare $M_\gamma$ to $M_{null}$ using a common $g$ (from Zellner's $g$-prior), 
...
\pause
Generally, we cannot perform model selection or comparison with improper
priors on model parameters. 
\pause
But, it turns out that if there are parameters common to all models, 
we can use improper priors on those parameters. 
\pause
For example, if $\sigma^2$ is the error variance and suppose there is an intercept $\alpha$,
we can place an improper prior on these parameters, 
typically $p(\alpha,\sigma^2)\propto 1/\sigma^2$, and not affect the Bayes Factors.
\end{frame}
...
library(BMS)
m0 = zlm(chirps ~ 1,           data = d, g = 'UIP') # g=n
m1 = zlm(chirps ~ scale(temp), data = d, g = 'UIP') # g=n
(bf = exp(m1$marg.lik - m0$marg.lik))               # Bayes factor
summary(m1)
@
\end{frame}
...
<<echo=TRUE, warning=FALSE, message=FALSE>>=
library(bayess)
m = BayesReg(d$chirps, d$temp, g = nrow(d)) # explanatory variables are scaled
@
\end{frame}
...
\]
\pause
Thus the empirical Bayes approach does not suffer from either paradox. 
\pause 
This empirical Bayes approach is model selection consistent 
if the true model is not the null model, 
but is inconsistent if it is.
\end{frame}
...
For this prior, we have $BF^{FB}(M_\gamma:M_{null}) \to \infty$ as 
$R_\gamma^2 \to 1$ and thus does not suffer from the previous paradoxes 
\pause 
and we have model selection consistency, 
i.e. $BF(M^*:M_\gamma) \stackrel{a.s.}{\longrightarrow} \infty$ 
for true model $M^*$ compared to any other model $M_\gamma$. 
