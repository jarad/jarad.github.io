...
We use the marginal likelihood as evidence in favor of the model, i.e. when comparing models those with higher marginal likelihoods should be preferred over the others
\end{frame}
...
= \frac{p(y|M)p(M)}{p(y)} 
\pause
= \frac{p(y|M)p(M)}{\sum_{j=1}^J p(y|M_j)p(M_j)} 
\]
where all quantities are scalars. 
\pause
The denominator simply renormalizes so that the posterior model probability
sums to 1. 

\vspace{0.1in} \pause

Thus higher marginal likelihood, $p(y|M)$, results in higher 
posterior model probability, $p(M|y)$. 
