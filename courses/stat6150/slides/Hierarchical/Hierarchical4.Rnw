...
Let $\{M_\gamma: \gamma \in \mG\}$ indicate a set of models for a particular data set $y$. 
\pause 
If $\Delta$ is a quantity of interest, e.g. effect size, a future observable, 
or the utility of a course of action, 
\pause 
...
Some data model and prior combinations have closed form marginal likelihoods,
e.g. 
\[ \begin{array}{rll}
Y|\theta &\sim Bin(n,\theta), \\
\theta &\sim Be(a,b) & \implies \pause \\
Y &\sim BetaBin(n,a,b)
\end{array} \]
\pause
and
\[ \begin{array}{rll}
Y|\mu,\sigma^2 &\sim N_n(\mu \mathrm{1}, \sigma^2 \mathrm{I}), \\
\mu|\sigma^2 &\sim N(m, \sigma^2v), \\
\sigma^2 &\sim IG(a,b) &\implies \pause \\
\pause
Y &\sim t_{2a}(m, b[v\mathrm{1}\mathrm{1}^\top + \mathrm{I}]/a),
\end{array} \]
i.e. a multivariate, location-scale t-distribution.

\vspace{0.1in} \pause

Unfortunately, the vast majority of marginal likelihoods are not available
in closed form. 
\end{frame}


\begin{frame}
\frametitle{Estimating the marginal likelihood}
...
& = E_{p(\theta)}[p(y|\theta)]
\pause \\
& \approx \frac{1}{M} \sum_{m=1}^M p\left(y|\theta^{(m)}\right), \quad
\theta^{(m)} \stackrel{ind}{\sim} p(\theta)
\end{array} \]
...
\[ \begin{array}{rl}
p(y) 
&= \int p(y|\theta) \frac{p(\theta)}{q(\theta)} q(\theta) d\theta 
\pause \\
&= E_{q(\theta)}\left[ \frac{p(\theta)}{q(\theta)} p(y|\theta)\right]
\pause \\
& \approx \frac{1}{M} \sum_{m=1}^M \frac{p\left(\theta^{(m)}\right)}{q\left(\theta^{(m)}\right)} 
p\left(y|\theta^{(m)}\right), \quad
\theta^{(m)} \stackrel{ind}{\sim} q(\theta)
\end{array} \]
\pause
which is an importance sampling approach.
\pause
An alternative estimator is 
\[ \begin{array}{rl}
p(y) 
&\approx \frac{\sum_{m=1}^M \frac{p\left(\theta^{(m)}\right)}{q\left(\theta^{(m)}\right)} p\left(y|\theta^{(m)}\right) }{\sum_{m=1}^M \frac{p\left(\theta^{(m)}\right)}{q\left(\theta^{(m)}\right)}}
\end{array} \]
\pause
which is biased but generally has lower variance
(see Robert and Casella, pg 94).
\end{frame}
...
% \begin{frame}
% Recall $p(y,\theta) = p(\theta|y)p(y)$
% \pause
% Laplace's approximation 
% (based on a Taylor series expansion around $\hat\theta_{MAP}$) 
% is 
% \[ 
% p(y,\theta) 
% \approx p(y,\hat\theta_{MAP}) \exp\left(-\frac{1}{2}(\theta-\hat\theta_{MAP}) A^{-1} (\theta-\hat\theta_{MAP}) \right)
% \pause
% = \frac{p(y,\hat\theta_{MAP})}{(2\pi)^{p/2} |A|^{1/2}} N(\theta; \hat\theta_{MAP, A)
% \]
% where $A^{-1}$ is the negative Hessian evaluated at $\hat\theta_{MAP}$ and $p$ is the dimension of $\theta$. 
% \pause
% Thus, we have 
% \[
% \theta|y \stackrel{\cdot}{\sim} N(\hat\theta_{MAP}, A)
% \]
% and 
% \[
% p(y) \approx 
...
\begin{frame}
\frametitle{Evaluating the marginal likelihood}

\small

Recall that as the sample size $n$ increases, the posterior converges to a normal distribution (Bernstein von Mises Theorem).
\pause
Let
\[
...
\[ 
E[\theta_i|y] = \sum_{\gamma\in\mG}  E[\theta_{\gamma,i}|y] p(M_\gamma|y)
\]
\pause
...
\]
There is a similar augmentation for logistic regression, see the \texttt{BayesLogit} R package and references therein. 
\pause
