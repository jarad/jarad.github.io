...
\frame{\maketitle
Adapted from \href{https://arxiv.org/abs/1206.1901}{Radford Neal's MCMC Using Hamiltonian Dynamics} in Handbook of Markov Chain Monte Carlo (2011).}
...
where 
\begin{itemize}
\item $U(\theta)$ is called the potential energy and will be defined to be the 
negative log probability density of the distribution for $\theta$ 
(plus any constant that is convenient) and 
\item $K(\omega)$ is called the kinetic energy and is usually defined as 
\[ 
K(\omega) = \omega^\top M^{-1} \omega / 2 
\pause
= m\, \omega^\top \mathrm{I}\, \omega / 2
\]
...
This form for $K(\omega)$ corresponds to the negative log probability density 
...
\frac{d\omega_i}{dt} = - \frac{U(\theta)}{\partial \theta_i}
= \frac{\partial \log p(\theta|y)}{\partial \theta_i}.
\]
\end{frame}
...
\begin{frame}
\frametitle{Properties of the Hamiltonian}

\begin{itemize}
\item Conservation of the Hamiltonian, i.e. $H(\theta,\omega)$ is constant
\item Reversibility 
...
i.e. the mapping $T_s$ from the state at time $t$, 
$(\theta(t),\omega(t))$, 
to the state at time $t + s$, 
$(\theta(t + s),\omega(t + s))$, 
is one-to-one, and hence as an inverse, $T_{-s}$.
...
If we apply the mapping $T_s$ to points in some region $R$ in 
$(\theta, \omega)$ space with
volume $V$, 
the image of $R$ under $T_s$ will also have volume $V$. 

\pause 

\begin{center}
\includegraphics[width=0.4\textwidth]{hamiltonian-volume-preservation}
\end{center}

{\tiny
Oltean, Marius \& Bonetti, Luca \& Spallicci di Filottrano, Alessandro D.A.M. \& F Sopuerta, Carlos. (2016). Entropy theorems in classical mechanics, general relativity, and the gravitational two-body problem. Physical Review D. 94. 10.1103/PhysRevD.94.064049.
}
...
H(\theta,\omega) = U(\theta) + K(\omega), 
\qquad 
K(\omega) = \sum_{i=1}^d \frac{\omega_i^2}{2m_i}.
\]
\pause
...
\pause
But the leapfrog method is still an approximation to the continuous-time
...
<<leapfrog-volume, fig.show='animate', interval=.1, fig.asp=1, fig.width=5, out.width='.5\\linewidth'>>=
...
Let $\mySigma=Var[\theta|y]$, then an optimal normal distribution for $\omega$ is $N(0,\mySigma^{-1})$. 
\pause
...
ggplot(r, aes(iteration,theta)) + 
  geom_line() + 
  facet_grid(L ~ e, labeller = label_both) +
  theme_bw() +
