...
\pause
This is a non-standard distribution, but is extremely close to a normal model
...
\pause
Since the top level distributional assumption is normal, 
we refer to this as a \alert{scale mixture of normals}. 
...
scale $\sqrt{bC/a}$, and variance $\frac{bC}{a-1}$. 
\pause 
...
\subsection{Discrete ixture distributions}
\begin{frame}
\frametitle{Dealing with discrete mixture distributions}
...
\[ \begin{array}{rl}
\pi_g' &= \frac{\pi \prod_{i=1}^{n_g} N(y_{ig}; 0, \sigma^2)}{\pi \prod_{i=1}^{n_g} N(y_{ig}; 0, \sigma^2) + (1-\pi) \prod_{i=1}^{n_g} N(y_{ig}; \mu, \phi_g+\sigma^2) }. \\
\phi_g' &= \left( \frac{1}{\phi_g} + \frac{n_g}{\sigma^2} \right)^{-1} \\
\mu_g' &= \phi_g' \left( \frac{1}{\phi_g}\mu + \frac{n_g}{\sigma^2}\overline{y}_g \right)
\end{array} \]
\end{frame}
...
\begin{frame}
\frametitle{Dealing with discrete mixture distributions (cont.)}
Let 
\[ \theta_g \ind \pi \delta_0 + (1-\pi) h(\cdot) \]
for some distribution $h()$ and $\pi \sim Beta(s,f)$.
